...
public class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.ViewHolder> {
    ...
    ArrayList<Post> homeItemsSearches = Post.getHomeItemsSearches();
    ...
    public void onBindViewHolder(@NonNull SearchAdapter.ViewHolder viewHolder, int i) {
        Post post = homeItemsSearches.get(i);
    ...
    public void convert(){
        for(int i = 0; i < homeItemsSearches.size(); i ++){
            for(int j = 0; j < posts.size(); j ++){
                if(homeItemsSearches.get(i).getId().equals(posts.get(j).getId())){
                    arr.add(j);
                }
            }
        }
